	...
	return (
		<div class="grid w-full justify-center gap-4 p-2 md:gap-6 md:p-4">
			...
			<div class="w-full text-center text-gray-700">
				{session.items.showMissing ? session.items.all.length : session.items.filtered.length} item{session.items.showMissing ? session.items.all.length === 1 ? '': 's' : session.items.filtered.length === 1 ? '' : 's'} Total
			</div>
		</div>
	);
});
